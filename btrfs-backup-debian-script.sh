...
# Function to stop systemd service with enhanced support
stop_systemd_service() {
    local service_name="$1"
    local user_name="${2:-}"
    local uid="${3:-}"

    if [[ -n "$user_name" && -n "$uid" ]]; then
        log "INFO" "Stopping user service: $service_name (user: $user_name, uid: $uid)"

        # Use runuser to execute as the specific user with proper environment
        if runuser -l "$user_name" -c "systemctl --user stop '$service_name'"; then
            log "INFO" "User service $service_name stopped successfully"
            return 0
        else
            log "WARNING" "Failed to stop user service $service_name"
            return 1
        fi
    else
        log "INFO" "Stopping system service: $service_name"

        if systemctl stop "$service_name"; then
            log "INFO" "System service $service_name stopped successfully"
            return 0
        else
            log "WARNING" "Failed to stop system service $service_name"
            return 1
        fi
    fi
}

# Function to start systemd service with enhanced support
start_systemd_service() {
    ...
    if [[ -n "$user_name" && -n "$uid" ]]; then
        log "INFO" "Starting user service: $service_name (user: $user_name, uid: $uid)"

        # Use runuser to execute as the specific user with proper environment
        if runuser -l "$user_name" -c "systemctl --user start '$service_name'"; then
            log "INFO" "User service $service_name started successfully"
            return 0
        else
            log "WARNING" "Failed to start user service $service_name"
            return 1
        fi
    else
        log "INFO" "Starting system service: $service_name"

        if systemctl start "$service_name"; then
            log "INFO" "System service $service_name started successfully"
            return 0
        else
            log "WARNING" "Failed to start system service $service_name"
            return 1
        fi
    fi
}
...
backup_redis() {
    local redis_config=$(yq eval '.backups.infrastructure.redis.config // false' "$CONFIG_FILE")

    if [[ "$redis_config" == "true" ]]; then
        log "INFO" "Starting Redis infrastructure backup..."

        local redis_temp_dir="$TEMP_DIR/infrastructure/redis"
        ...
        # Skip if /etc backup is enabled (avoid duplication)
        if [[ "$etc_enabled" == "true" ]]; then
            log "INFO" "Skipping Redis config backup (covered by /etc backup): /etc/redis"
        else
            if [[ -d "/etc/redis" ]]; then
                local config_backup_dir="$redis_temp_dir/config"
                mkdir -p "$config_backup_dir"

                if rsync -av /etc/redis/ "$config_backup_dir/"; then
                    log "INFO" "Redis configuration backup completed"
                else
                    handle_error "Failed to backup Redis configuration"
                fi
            else
                log "WARNING" "Redis configuration directory /etc/redis not found"
            fi
        fi

        # Copy Redis backups to each backup disk
        copy_to_backup_disks "$redis_temp_dir" "infrastructure/redis"

        log "INFO" "Redis infrastructure backup completed"
    else
        log "INFO" "Redis infrastructure backup disabled"
    fi
}

# Function to backup Meilisearch
backup_meilisearch() {
    local meilisearch_config=$(yq eval '.backups.infrastructure.meilisearch.config // false' "$CONFIG_FILE")
    local meilisearch_data=$(yq eval '.backups.infrastructure.meilisearch.data // false' "$CONFIG_FILE")

    if [[ "$meilisearch_config" == "true" ]] || [[ "$meilisearch_data" == "true" ]]; then
        log "INFO" "Starting Meilisearch infrastructure backup..."

        local meilisearch_temp_dir="$TEMP_DIR/infrastructure/meilisearch"
        mkdir -p "$meilisearch_temp_dir"

        # Backup Meilisearch configuration if enabled
        if [[ "$meilisearch_config" == "true" ]]; then
            log "INFO" "Backing up Meilisearch configuration..."
            local etc_enabled=$(yq eval '.backups.etc // false' "$CONFIG_FILE")

            # Skip if /etc backup is enabled (avoid duplication)
            if [[ "$etc_enabled" == "true" ]]; then
                log "INFO" "Skipping Meilisearch config backup (covered by /etc backup): /etc/meilisearch"
            else
                if [[ -d "/etc/meilisearch" ]]; then
                    local config_backup_dir="$meilisearch_temp_dir/config"
                    mkdir -p "$config_backup_dir"

                    if rsync -av /etc/meilisearch/ "$config_backup_dir/"; then
                        log "INFO" "Meilisearch configuration backup completed"
                    else
                        handle_error "Failed to backup Meilisearch configuration"
                    fi
                else
                    log "WARNING" "Meilisearch configuration directory /etc/meilisearch not found"
                fi
            fi
        fi

        # Backup Meilisearch data if enabled
        if [[ "$meilisearch_data" == "true" ]]; then
            log "INFO" "Backing up Meilisearch data..."
            if [[ -d "/var/lib/meilisearch" ]]; then
                local data_backup_dir="$meilisearch_temp_dir/data"
                mkdir -p "$data_backup_dir"

                if rsync -av /var/lib/meilisearch/ "$data_backup_dir/"; then
                    log "INFO" "Meilisearch data backup completed"
                else
                    handle_error "Failed to backup Meilisearch data"
                fi
            else
                log "WARNING" "Meilisearch data directory /var/lib/meilisearch not found"
            fi
        fi

        # Copy Meilisearch backups to each backup disk
        copy_to_backup_disks "$meilisearch_temp_dir" "infrastructure/meilisearch"

        log "INFO" "Meilisearch infrastructure backup completed"
    else
        log "INFO" "Meilisearch infrastructure backup disabled"
    fi
}
...
backup_services() {
    ...
    for ((i = 0; i < services_count; i++)); do
        ...
        local systemd_service=$(yq eval ".backups.services[$i].systemd.name" "$CONFIG_FILE")
        local user_name=$(yq eval ".backups.services[$i].systemd.podman.user // \"\"" "$CONFIG_FILE")
        local uid=$(yq eval ".backups.services[$i].systemd.podman.uid // \"\"" "$CONFIG_FILE")

        if [[ -n "$systemd_service" && "$systemd_service" != "null" ]]; then
            log "INFO" "Stopping service: $systemd_service"
            if stop_systemd_service "$systemd_service" "$user_name" "$uid"; then
                stopped_services+=("$systemd_service:$user_name:$uid")
    ...
    for ((i = 0; i < services_count; i++)); do
        local service_label=$(yq eval ".backups.services[$i].label" "$CONFIG_FILE")
        local compose_path=$(yq eval ".backups.services[$i].containers.compose // \"\"" "$CONFIG_FILE")
        ...
        # Backup Container Compose if specified
        if [[ -n "$compose_path" && "$compose_path" != "null" ]] && [[ -d "$compose_path" ]]; then
            log "INFO" "Backing up Container Compose for $service_label: $compose_path"
            local compose_backup_dir="$service_temp_dir/containers_compose"
            mkdir -p "$compose_backup_dir"

            # Copy docker-compose files
            find "$compose_path" -maxdepth 1 \( -name "docker-compose.yml" -o -name "docker-compose.yaml" \) -exec cp {} "$compose_backup_dir/" \; 2>/dev/null

            # Copy .env files
            find "$compose_path" -maxdepth 1 -name ".env*" -exec cp {} "$compose_backup_dir/" \; 2>/dev/null

            # Copy additional config files
            for config_file in "Dockerfile" ".dockerignore" "docker-compose.override.yml" "docker-compose.override.yaml"; do
                if [[ -f "$compose_path/$config_file" ]]; then
                    cp "$compose_path/$config_file" "$compose_backup_dir/" 2>/dev/null
                fi
            done

            log "INFO" "Container Compose backup completed for $service_label"
        fi
    ...
    for service_info in "${stopped_services[@]}"; do
        IFS=':' read -r service_name user_name uid <<<"$service_info"
        log "INFO" "Starting service: $service_name"
        if start_systemd_service "$service_name" "$user_name" "$uid"; then
            log "INFO" "Service $service_name started"
        else
            handle_error "Failed to start service $service_name"
        fi
    done
